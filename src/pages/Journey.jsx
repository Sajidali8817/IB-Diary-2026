...
const TaskNode = ({ task, index, isCurrent, isLocked, onClick }) => (
    ...
    >
        <div className={`
            w-16 h-16 rounded-[2rem] flex items-center justify-center dark:text-white text-white shadow-2xl transition-all border-4
            ${task.completed ? 'bg-indigo-600 border-emerald-400 shadow-indigo-900' :
                isCurrent ? 'bg-blue-500 border-white animate-pulse shadow-blue-900' :
                    'dark:bg-slate-800/50 bg-slate-200 dark:border-slate-700 border-slate-300 opacity-50 grayscale shadow-none dark:text-white text-slate-500'}
        `}>
        ...
        {!isLocked && (
            <div className={`
                absolute top-1/2 -translate-y-1/2 whitespace-nowrap px-4 py-2 dark:bg-slate-900 bg-white dark:border-white/10 border-slate-200 border rounded-xl dark:text-white text-slate-900 text-[10px] font-black uppercase tracking-[0.2em] transition-all
    ...
    return (
        ...
        >
            <path
                d={pathD}
                fill="none"
                className="dark:stroke-white stroke-slate-400 dark:opacity-10 opacity-30"
                strokeWidth="4"
...
const Journey = () => {
    ...
    return (
        <div className="min-h-screen dark:bg-slate-950 bg-slate-50 pb-32 relative overflow-hidden transition-colors duration-300">
            {/* Background Texture */}
            <div className="absolute inset-0 opacity-5 pointer-events-none">
                <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(#fff_1px,transparent_1px)] [background-size:24px_24px] dark:block hidden" />
                <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(#000_1px,transparent_1px)] [background-size:24px_24px] dark:hidden block" />
            </div>
            ...
            <div className="dark:bg-slate-900/80 bg-white/80 backdrop-blur-xl sticky top-0 z-30 dark:border-white/5 border-slate-200 border-b">
                <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-6">
                    <div>
                        <h1 className="text-2xl font-black dark:text-white text-slate-900 font-outfit leading-none tracking-tight uppercase italic">Today's Journey</h1>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="text-right">
                                ...
                                <p className="text-xl font-black dark:text-white text-slate-900 font-outfit leading-none">{progress}%</p>
                            </div>
                            <div className="relative w-12 h-12 rounded-full dark:bg-slate-900 bg-white flex items-center justify-center dark:border-white/5 border-slate-200 border">
                                <svg className="absolute inset-0 w-full h-full -rotate-90 p-1">
                                    <circle
                                        cx="24" cy="24" r="20"
                                        fill="none" stroke="currentColor"
                                        strokeWidth="3" className="dark:text-slate-800 text-slate-200"
                                    />
                                    <motion.circle
                                        cx="24" cy="24" r="20"
                                        fill="none" stroke="currentColor"
            ...
            <div className="p-6">
                ...
                <div className="relative py-10 min-h-[800px] max-w-2xl mx-auto">
                    ...
                    <div className="flex flex-col-reverse gap-32 relative z-10">
                        ...
                        <div className="self-center">
                            <div className="w-16 h-16 dark:bg-slate-800 bg-white dark:border-white/10 border-slate-200 border-2 rounded-2xl flex flex-col items-center justify-center rotate-6 shadow-2xl">
                ...
                {todayTasks.length === 0 && (
                    <div className="max-w-md mx-auto">
                        <div className="dark:bg-slate-900/40 bg-white backdrop-blur-xl dark:border-white/10 border-slate-200 border p-8 rounded-[2.5rem] flex flex-col items-center text-center gap-6 shadow-xl">
                            <div className="w-16 h-16 rounded-2xl dark:bg-blue-500/10 bg-blue-50 text-blue-500 flex items-center justify-center">
                                <MdLightbulbOutline size={32} />
                            </div>
                            <div>
                                <p className="dark:text-white text-slate-900 font-black text-lg font-outfit">Your path is quiet today</p>
                ...
                {selectedTask && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/90 backdrop-blur-xl">
                        <motion.div
                            ...
                            className="dark:bg-slate-900 bg-white dark:border-white/10 border-slate-200 border rounded-[3rem] w-full max-w-md p-10 shadow-3xl"
                        >
                            <div className="flex justify-between items-start mb-8">
                                ...
                                <button onClick={() => setSelectedTask(null)} className="p-2 dark:bg-slate-800 bg-slate-100 rounded-full text-slate-500 dark:hover:text-white hover:text-slate-900 transition-colors">
                                    <MdClose size={24} />
                                </button>
                            </div>

                            <h2 className="text-3xl font-black dark:text-white text-slate-900 font-outfit mb-4 uppercase tracking-tighter">{selectedTask.title}</h2>
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-10">
                                <div className="dark:bg-slate-950 bg-slate-50 p-5 rounded-3xl dark:border-white/5 border-slate-200 border">
                                ...
                                <div className="dark:bg-slate-950 bg-slate-50 p-5 rounded-3xl dark:border-white/5 border-slate-200 border">
                                    ...
                                    <p className="text-sm font-black dark:text-white text-slate-900">{selectedTask.dueTime || 'All Day'}</p>
                                </div>
                            </div>
