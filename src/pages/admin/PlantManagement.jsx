        ...
        <div className="min-h-screen bg-slate-950 p-6 pb-24">
            ...
            <header className="flex items-center gap-4 mb-8">
                ...
                <div className="flex-1">
                    <h1 className="text-2xl font-black text-white font-outfit">Plant Management</h1>
            ...
            <div className="relative mb-8">
                ...
                <input
                    ...
                    className="w-full bg-slate-900 border border-white/5 rounded-2xl py-4 pl-12 pr-4 text-white outline-none focus:border-emerald-500 transition-all font-bold placeholder:text-slate-700"
                />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <AnimatePresence>
                    {loading ? (
                        [...Array(6)].map((_, i) => <div key={i} className="h-28 bg-slate-900/40 rounded-3xl animate-pulse" />)
                    ) : (
                        filteredPlants.map((plant, index) => (
                            <motion.div
                                ...
                                className="bg-slate-900/40 backdrop-blur-xl border border-white/5 rounded-3xl p-6 group hover:bg-slate-900/60 transition-all border-l-4 border-l-emerald-500/50"
                            >
                                <div className="flex items-start justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h3 className="text-white font-black text-lg leading-tight">{plant.plant_name}</h3>
                                    ...
                                    <div className="flex gap-1">
                                        <button onClick={() => openModal(plant)} className="p-2 hover:bg-white/5 rounded-xl text-slate-400 hover:text-blue-400 transition-all">
                                            <MdEdit size={18} />
                                        </button>
                                        <button onClick={() => handleDelete(plant.id)} className="p-2 hover:bg-white/5 rounded-xl text-slate-400 hover:text-red-400 transition-all">
                                            <MdDelete size={18} />
                                        </button>
                                    </div>
                                </div>
                ...
                {isModalVisible && (
                    <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-black/60 backdrop-blur-md">
                        <motion.div
                            ...
                            className="bg-slate-900 border border-white/10 rounded-[2.5rem] w-full max-w-md p-8 shadow-3xl"
                        >
...
const InputField = ({ label, placeholder, value, onChange, type = "text", required = false }) => (
    <div className="space-y-2">
        ...
        <input
            ...
            required={required}
            className="w-full bg-slate-950 border border-white/5 rounded-2xl p-4 text-white font-bold outline-none focus:border-emerald-500 transition-all placeholder:text-slate-800"
        />
    </div>
);
